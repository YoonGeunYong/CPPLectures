...
# 프렌즈 함수 (외부 함수) 선언 
클래스 외부에서 정의된 함수가 클래스 멤버를 접근할 수 있도록 하기 위해서 클래스 외부 함수를 클래스 내에서 프렌드 키워드를 사용하여 아래와 같이 선언한다.
...
직접 접근하여 ```Rect```클래스의 두 객체를 비교하고 있다. 
...
* 프렌즈 함수와 프레드 클래스의 선언은 클래스 내의 어디에서도 가능하다.
